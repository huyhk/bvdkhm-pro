async function loadDeck(){
  ...
  try{
    const res = await fetch('sections/manifest.json?v=2.0-m5-rc1', {cache:'no-store'});
    if(!res.ok) throw new Error('Không tải được manifest');
    const manifest = await res.json();

    const fragments = await Promise.all(manifest.map(async item => {
      const r = await fetch(item.file + '?v=2.0-m5-rc1', {cache:'no-store'});
